...
function caught_speeding(speed, is_birthday) {
  let ticket = 0;
  let noTicketSpeed = 60;
  let smallTicketSpeed = 80;
  let bigTicketSpeed = 81;
  ...
  if (speed <= noTicketSpeed) {
    ticket = 0;
  } else if (speed > noTicketSpeed && speed <= smallTicketSpeed) {
    ticket = 1;
  } else if (speed > bigTicketSpeed) {
    ticket = 2;
  }
...
function makeBricks(small, big, goal) {
  //Code Goes Here
  if (goal % 5 >= 0 && goal % 5 - small <= 0 && small + 5 * big >= goal) {
    return true;
  } else {
    return false;
  }
